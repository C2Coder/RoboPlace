...
class Screen:
    ...
    def init():
        Screen.pixels = [['white' for i in range(
            Game.size)] for j in range(Game.size)]
        req = requests.get(url=server_ip + '/get_pixels')
        response = req.text
        for y in range(100):
            for x in range(100):
                Screen.pixels[x][y] = Screen.colors[Screen.chars.index(
                    response[(y*100)+x])]
    ...
    def update(surface):
        for y in range(Game.size):
            for x in range(Game.size):
                pygame.draw.rect(surface, Screen.COLOR[Screen.pixels[x][y]], (
                    x*Screen.pixel_size, y*Screen.pixel_size, Screen.pixel_size, Screen.pixel_size))
